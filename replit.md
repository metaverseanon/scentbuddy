...
- `artifacts/scentbuddy/supabase/migrations/2026_07_scent_quiz_profile.sql` — adds a `jsonb` `scent_quiz` column to `profiles` so each user's full onboarding quiz answers (the redesigned 16-question flow in `constants/quiz.ts` → `QuizResults`, incl. struggles/goals/budget/personality/etc.) persist per user for a future per-user "smart picks" recommendation algorithm. JSONB (not promoted columns) is intentional — suggestions are computed per user from one profile row, so the blob can evolve without a migration each time a question is added. New users persist via `AuthProvider.provisionNewUser`; returning-user retakes now use the SAME rich 16-question flow via the shared `components/ScentQuiz.tsx` component, surfaced to existing users through the standalone `app/scent-quiz.tsx` route (opened from the For You "Take Quiz" button), persisting via `updateProfile({ scent_quiz, favorite_note })`. The old divergent 4-question inline `QuizModal` was removed — `ScentQuiz` is the single source of truth for the quiz, reused by both `app/onboarding.tsx` and the retake route. Idempotent (`add column if not exists`) — paste into the Supabase SQL editor to apply.
